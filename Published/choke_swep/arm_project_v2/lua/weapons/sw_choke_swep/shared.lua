...
function SWEP:PrimaryAttack()
	...
	if  not npc:IsNPC() and not npc:IsNextBot() and not npc:IsPlayer() or distance > 450 then return end
	
	 
	for i = 0, npc:GetBoneCount() - 1 do
		print( i, npc:GetBoneName( i ) )
	end
...
function SWEP:SecondaryAttack()
	...
	if self.ragdoll ~= nil then 
		...
		timer.Simple(0.2,function()
			...
			phys_obj:ApplyForceOffset((player:GetRight() )*10000, phys_obj:GetPos() + Vector(0,40,0))
...
function SWEP:Think()
	...
	if CurTime() - self.slowTime > 0.5 then
		for k , v in pairs(self.ragdollToNPC) do
			...
			print (k)
			print (v)
			hook.Add("Think", "RagdollToNPC_"..tostring(k), function()
				...
				if (CurTime() - k.timer > 2) and k and v then
					...
					k:Remove()
					hook.Remove("Think", "RagdollToNPC_"..tostring(k))
				end
	...
	if not self.Owner:KeyDown(IN_ATTACK) and not self.Owner:KeyDown(IN_ATTACK2) then
		...
		if self.ragdoll then
			...
			if self.ragdollHP > 0 then
				if not self.isPlayer then
					self.ragdollToNPC[self.ragdoll] = {npc = self.npc , model = self.npcModel}
				else
		...
		if IsValid(phys_obj) then
			...
			if( ragdoll_hand_L or ragdoll_hand_R ) and self.ragdollHP > 0 then
				...
				-- else
					...
					ragdoll_hand_L:SetPos(phys_obj:GetPos() - player:GetAimVector()*10 + player:GetRight()*2)
					ragdoll_hand_L:SetAngles(angle)
					ragdoll_hand_R:SetPos(phys_obj:GetPos() - player:GetAimVector()*10 - player:GetRight()*4)
			...
			phys_obj:SetPos(player:EyePos() + player:GetAimVector() * 300)
			...
			if self.ragdollHP > 0 then
				...
				self.ragdoll:AddCallback("PhysicsCollide", function(obj,data)
					...
					if (speed > 700)  and (CurTime() - self.lastDecalTime > 0.5) and data.HitEntity ~= self.ragdoll then
						...
						effect:SetScale(1)
						util.Effect("BloodImpact", effect)
						self.ragdollHP = self.ragdollHP - 26
			...
			elseif self.isPlayer then
				...
				print ("PLAYER THAT DIES", self.npc)
