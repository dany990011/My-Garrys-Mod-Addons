		...
		if IsValid(phys_obj) then
			...
			if( ragdoll_hand_L or ragdoll_hand_R ) and self.ragdollHP > 0 then
				...
				-- else
					...
					ragdoll_hand_L:SetPos(player_hand_pos + player:GetAimVector() + player:GetRight()*2)
					ragdoll_hand_L:SetAngles(angle)
					ragdoll_hand_R:SetPos(player_hand_pos + player:GetAimVector() - player:GetRight()*4)
			...
			phys_obj:SetPos(player:EyePos() + player:GetAimVector() * 25)
