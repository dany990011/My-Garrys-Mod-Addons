...
SWEP.BobScale = 1
SWEP.SwayScale = 1
SWEP.Instructions = "Right click to punch, Scroll wheel to set distance"
	...
	if not self.Owner:KeyDown(IN_ATTACK) and not self.Owner:KeyDown(IN_ATTACK2) then
		...
		if self.ragdoll then
			...
			if self.ragdollHP and self.ragdollHP > 0 then
				print("RAGDOLL HP IS "..self.ragdollHP)
				print("THEIS IS RAG TO NPC CALL ", self.npc)
				if not self.isPlayer then
