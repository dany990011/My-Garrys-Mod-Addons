            ...
            for k,v in pairs(self.pieces) do 
                ...
                if not v.ok then 
                    ...
                    if gridPosW[v.gridPos - 10] == nil  or  gridPosW[v.gridPos - 10].occupied then
                        ...
                        effectData:SetOrigin(v:GetPos() + Vector(0,0,-17.5)) 
    ...
    if CLIENT then
        ...
        hook.Add("PostDrawOpaqueRenderables", "DrawEntityText", function()
            ...
            if IsValid(entity) then
                ...
                if self.cl_displace > 0.5 and CurTime() > self.cl_sampleTime then
                    self.cl_displace = self.cl_displace - (self.cl_displace*2^(-(CurTime() - self.cl_displaceTime)))/10 -- i dont know math, sorry
                    self.cl_sampleTime = CurTime() + 0.001
                    
                elseif self.cl_displace < 0.5 then
                    self.cl_displace = 0
                end
                ...
                if self.cl_gameOver then
                    ...
                    cam.Start3D2D(entity:GetPos() + Vector(0,0,100) + entity:GetForward() * 30 , angle , 1)  -- Start drawing in 3D2D space
                    
                        draw.DrawText("GAME OVER", "GModNotify", -textWidth * 0.5, -textHeight * 0.5, Color(224,0,0), TEXT_ALIGN_LEFT) 
