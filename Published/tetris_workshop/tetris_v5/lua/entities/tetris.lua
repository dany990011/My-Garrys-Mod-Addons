...
if CLIENT then
    ...
    hook.Add( "PopulateToolMenu", "TetrisMenuSettings", function() -- Thanks Zaurzo :)
        spawnmenu.AddToolMenuOption( "Utilities", "Tetris", "Custom_Menu", "Tetris Options", "", "", function( panel )
            ...
            panel:CheckBox("Remove Old Blocks on Remove", "tetris_remove_old_blocks")
            panel:NumSlider( "Game Speed", "tetris_game_speed", 1, 8 )

            local colorDropdown = panel:ComboBox("Block Material", "tetris_block_material")
            for _, option in pairs(materialOptions) do
                colorDropdown:AddChoice(option.text, option.value)
            end

            panel:CheckBox("Enable Random Materials", "tetris_random_materials")
        end )
    end )
...
function ENT:Think()
    ...
    if SERVER then
        ...
        for k,v in pairs(positions) do 
            ...
            local distX = dist.x
            local distY = dist.y
            local distZ = self:GetPos().z + gridPosW[206].pos.z - self[k.."Border"]:GetPos().z
